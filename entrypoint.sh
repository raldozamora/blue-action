...
npx bluemachine -t ${TM_TOKEN_KEY} snapshot create --repo-id ${1} --file ${2} --json-output ${FILE_S_CREATE}
if test -f "$FILE_S_CREATE"; then
    # analyze snapshot
    SNAPSHOT_ID=$(jq '.ID' $FILE_S_CREATE)
    echo "Snapshot Id : $SNAPSHOT_ID"
    FILE_S_ANA="/action_work_folder/s_ana.json"
    npx bluemachine -t ${TM_TOKEN_KEY} snapshot analyze --snapshot-id=${SNAPSHOT_ID} --json-output ${FILE_S_ANA}
    if test -f "$FILE_S_ANA"; then
        ...
        npx bluemachine -t ${TM_TOKEN_KEY} analyses report --analysis-id=${ANA_ID} --json-output ${FILE_A_R_OUTPUT}        
